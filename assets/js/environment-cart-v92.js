(() => {
  ...
  function add(item) {
    ...
    if (found) {
      ...
      rows.push({
        ...
        serviceType: item.serviceType || item.board || '环境检测',
        board: item.board || item.serviceType || '环境检测',
  ...
  function ensureDock() {
    ...
    dock.innerHTML = '<span>需求清单</span><b data-env-cart-count>0</b>';
